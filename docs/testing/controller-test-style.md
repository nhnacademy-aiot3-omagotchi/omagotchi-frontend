...
- 클래스는 `@WebMvcTest` 다음에 개별 설정 어노테이션을 작성한다.
- REST Docs 테스트는 `FrontendRestDocsTestSupport`를 상속해 REST Docs와 MVC 공통 설정을
  재사용한다.
...
class SpaceBffDocumentationTest extends FrontendRestDocsTestSupport {
    ...
    void getsSpaces() throws Exception {
        // Given: 인증 세션과 빈 공간 목록
        MockHttpSession session = authenticatedSession();
        given(spaceBffService.findAll(any(HttpServletRequest.class))).willReturn(List.of());

        // When & Then
        mockMvc.perform(get("/bff/v1/spaces").session(session))
                .andExpect(status().isOk())
                .andExpect(content().json("[]"))
                .andDo(document("spaces/list"));
