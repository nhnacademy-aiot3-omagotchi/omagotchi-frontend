...
아래 두 항목은 이후 작업에서 완료했다. 경과는 [후속 경과](#후속-경과-2026-09-14-기준) 참고.
...
- Ollama Qwen 2.5 모델 선택은 배선은 됐지만 실제 응답을 받아본 적이 없다 (응답이 느려서 이
  세션에서도 검증하지 못함).

## 후속 경과 (2026-09-14 기준)

이 절은 위 기록 이후 확인·변경된 사실만 덧붙인다. 위 본문은 작성 시점의 기록이므로
고치지 않는다.

- **전 구간 왕복은 운영에서 확인됐다** — 단 Gemini만이다.
- **~~Ollama도 실제 응답을 받았다~~ (2026-09-14 정정)** — 이 절의 이전 판에 있던 문장은
  오기였다. 대화 기록 문제를 재현하며 *의도적으로 실패를 유도한* 기록을 성공으로 잘못
  읽었다. Ollama는 이 시점까지 한 번도 답을 준 적이 없다.
  - 원인은 `learning-service`의 전역 읽기 타임아웃 2초가 Ollama 호출에도 걸린 것이었고,
    2026-09-04에 전용 HTTP 클라이언트로 분리해 해결했다.
  - **그러나 고친 뒤에도 상한(50초) 안에 답하지 못한다.** 남은 지연의 정체는 아직 모른다.
- **다만 경로가 위 서술과 다르다.** 프론트 BFF는 게이트웨이를 거치지 않고 Discovery로
  `learning-service`를 직접 호출한다(`LEARNING_SERVICE_BASE_URL=lb://learning-service`,
  최상위 ADR 0010이 결정한 방식). 게이트웨이의 `/api/v1/chat` 라우트는 **외부
  클라이언트용 공개 경로**로 별도 존재하며 BFF는 쓰지 않는다. 위 "다음 작업"의
  "BFF → 게이트웨이 → learning-service"는 당시의 이해였고 실제 구현과 다르다.
- 이후 별도 작업에서 AI 채팅 전용 읽기 타임아웃(30초 → 2026-09-05에 50초)과 답변 평문
  렌더링이 추가됐다.
  현재 계약은 [AI 도우미 연동 Prompt](../../prompt/AI-연동.md)와
  [Home AI 도우미 구현 Prompt](../../prompt/AI-도우미.md)가 소유한다.