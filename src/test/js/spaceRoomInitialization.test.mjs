...
test("linked Telegram users can open their vacancy alert applications", async () => {
    const api = {
        spaces: {
            async list() {
                return [{
    ...
    await withSpaceRoom(async (spaceRoom) => {
        ...
        assert.doesNotMatch(root.innerHTML, /space-room-vacancy-alerts__list/);
        ...
        assert.match(root.innerHTML, /내 알림 신청/);
        assert.match(root.innerHTML, /1건/);
        assert.match(root.innerHTML, /공용 회의실/);
        assert.match(root.innerHTML, /대기 중/);
    }, { activeTab: "meeting" }, {}, api);
});

test("unlinked Telegram users see the unlinked status", async () => {
    const api = {
        spaces: {
            async list() {
                return [];
    ...
    await withSpaceRoom(async (spaceRoom) => {
        ...
        assert.match(root.innerHTML, /텔레그램 연동 안됨/);
        assert.doesNotMatch(root.innerHTML, /내 알림 신청 보기/);
    }, { activeTab: "meeting" }, {}, api);
});

test("linked Telegram users with notifications disabled see the disabled status", async () => {
    const api = {
        spaces: {
            async list() {
                return [];
    ...
    await withSpaceRoom(async (spaceRoom) => {
        ...
        assert.match(root.innerHTML, /텔레그램 알림 꺼짐/);
        assert.doesNotMatch(root.innerHTML, /내 알림 신청 보기/);
    }, { activeTab: "meeting" }, {}, api);
});
...
test("a meeting collision refreshes the room and explains that another user won", async () => {
    let occupied = false;
    let listRequests = 0;
    let startRequests = 0;
    const api = {
    ...
    await withSpaceRoom(async (spaceRoom) => {
        ...
        assert.match(root.innerHTML, /사용 중/);
        assert.match(toast.textContent, /다른 사용자가 먼저 회의실 사용을 시작했습니다/);
...
test("library uses the shared current location and keeps its action under operational status", async () => {
    ...
    const api = {
        ...
        spaces: {
            async list() {
                return [
                    { spaceId: 101, name: "3기 실습실 A", type: "LAB", capacity: 30, operationalStatus: "ACTIVE", cohortId: 3 },
                    { spaceId: 401, name: "도서관", type: "STUDY", capacity: 1, currentPresenceCount: 7, operationalStatus: "ACTIVE", cohortId: null }
                ];
            },
            async getMyVacancyAlerts() {
                return [];
    ...
    await withSpaceRoom(async (spaceRoom) => {
        ...
        assert.match(root.innerHTML, /현재 7명 이용 중/);
        assert.doesNotMatch(root.innerHTML, /7\s*\/\s*1명|정원 마감/);
