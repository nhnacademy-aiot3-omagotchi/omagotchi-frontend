...
const MOCK_OCCUPANCIES = Object.freeze([
  {
    occupancyId: "mock-occupancy-a",
    spaceName: "회의실A",
    owner: "asdf@asdf.com",
    startedAt: "오늘 13:10",
    remainingTime: "1:25:00",
    status: "점유 중",
    participants: [
      { id: "mock-a-1", name: "asdf@asdf.com", role: "점유자", status: "참여 중", joinedAt: "13:10" },
      { id: "mock-a-2", name: "김학습", role: "참여자", status: "참여 중", joinedAt: "13:14" },
      { id: "mock-a-3", name: "이오마", role: "참여자", status: "참여 중", joinedAt: "13:22" }
    ]
  },
  {
    occupancyId: "mock-occupancy-b",
    spaceName: "회의실B",
    owner: "aaaa@asdf.com",
    startedAt: "오늘 14:03",
    remainingTime: "0:42:00",
    status: "점유 중",
    participants: [
      { id: "mock-b-1", name: "aaaa@asdf.com", role: "점유자", status: "참여 중", joinedAt: "14:03" },
      { id: "mock-b-2", name: "박참여", role: "참여자", status: "참여 중", joinedAt: "14:08" }
    ]
  },
  {
    occupancyId: "mock-occupancy-c",
    spaceName: "회의실C",
    owner: "manager@example.com",
    startedAt: "오늘 14:20",
    remainingTime: "2:08:00",
    status: "점유 중",
    participants: [
      { id: "mock-c-1", name: "manager@example.com", role: "점유자", status: "참여 중", joinedAt: "14:20" },
      { id: "mock-c-2", name: "최스터디", role: "참여자", status: "참여 중", joinedAt: "14:25" },
      { id: "mock-c-3", name: "정러닝", role: "참여자", status: "자리 비움", joinedAt: "14:31" },
      { id: "mock-c-4", name: "한오마", role: "참여자", status: "참여 중", joinedAt: "14:37" }
    ]
  }
]);
...
function ParticipantDialog({ occupancy, onClose }) {
  return (
    <div className="sensor-dialog-backdrop">
        ...
        <form onSubmit={(event) => { event.preventDefault(); onClose(); }}>
          ...
          <div className="space-participant-list">
            {occupancy.participants.map((participant) => (
              <article key={participant.id} className="space-participant-item">
                <div><strong>{participant.name}</strong><span>{participant.joinedAt} 입장</span></div>
                <div className="space-participant-badges"><span className="space-role-badge">{participant.role}</span><span className={`space-presence-badge${participant.status === "자리 비움" ? " is-away" : ""}`}>{participant.status}</span></div>
              </article>
            ))}
          </div>
  ...
  return (
    <div className="sensor-dialog-backdrop">
      <div className="sensor-dialog" role="alertdialog" aria-modal="true" aria-labelledby="force-end-dialog-title" aria-describedby="force-end-dialog-description">
        <form onSubmit={(event) => { event.preventDefault(); onConfirm(occupancy.occupancyId); }}>
          <header>
          ...
          <footer><div><button type="button" className="sensor-secondary-button" onClick={onClose}>취소</button><button type="submit" className="sensor-primary-button is-danger">강제 종료</button></div></footer>
        </form>
      </div>
    </div>
  );
...
function OccupancyManagement({ occupancies, onShowParticipants, onForceEnd }) {
  const participantCount = occupancies.reduce((sum, occupancy) => sum + occupancy.participants.length, 0);

  return (
    <div className="space-occupancy-panel">
      <header className="space-management-header"><div><span>PARTICIPANT MANAGEMENT</span><h2>참여자 관리</h2><p>현재 회의실 점유와 참여 현황을 확인하고 관리합니다.</p></div><span className="space-mock-label">MOCK DATA</span></header>
      ...
      <div className="space-table-wrap">
        <table className="sensor-table space-occupancy-table">
          ...
          <tbody>{occupancies.map((occupancy) => (
            <tr key={occupancy.occupancyId}>
              ...
              <td data-label="점유자">{occupancy.owner}</td>
              <td data-label="현재 참여자 수">{occupancy.participants.length}명</td>
              <td data-label="시작 시각">{occupancy.startedAt}</td>
              <td data-label="남은 시간"><strong className="space-remaining-time">{occupancy.remainingTime}</strong></td>
              <td data-label="상태"><span className="sensor-status is-active">{occupancy.status}</span></td>
        ...
        {occupancies.length === 0 && <div className="space-occupancy-empty"><strong>현재 점유 중인 회의실이 없습니다.</strong><p>강제 종료한 점유는 이 목록에서 제거됩니다.</p></div>}
      </div>
    </div>
  );
}

export function SpaceWorkspace({ spaces = [], selectedCohortId = null, loading = false, error = null, onRetry, onSave, onChangeStatus, onDelete, onChangeCohort, embedded = false }) {
  const [dialogSpace, setDialogSpace] = useState(undefined);
  const [actionDialog, setActionDialog] = useState(null);
  const [activeTab, setActiveTab] = useState("spaces");
  const [mockOccupancies, setMockOccupancies] = useState(() => MOCK_OCCUPANCIES.map((occupancy) => ({ ...occupancy, participants: occupancy.participants.map((participant) => ({ ...participant })) })));
  ...
  function forceEnd(occupancyId) {
    setMockOccupancies((current) => current.filter((occupancy) => occupancy.occupancyId !== occupancyId));
    setForceEndOccupancy(null);
  }
  ...
  return (
    <main className={`sensor-story-canvas${embedded ? " is-embedded" : ""}`}>
          ...
          <div className="space-workspace-tabs">
            <Tabs.Root value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="sensor-content-shell space-content-shell">
                ...
                <Tabs.Content value="participants"><OccupancyManagement occupancies={mockOccupancies} onShowParticipants={setSelectedOccupancy} onForceEnd={setForceEndOccupancy} /></Tabs.Content>
              </div>
            </Tabs.Root>
          </div>
        )}
      ...
      {selectedOccupancy && <ParticipantDialog occupancy={selectedOccupancy} onClose={() => setSelectedOccupancy(null)} />}
      {forceEndOccupancy && <ForceEndDialog occupancy={forceEndOccupancy} onClose={() => setForceEndOccupancy(null)} onConfirm={forceEnd} />}
    </main>
  );
}
